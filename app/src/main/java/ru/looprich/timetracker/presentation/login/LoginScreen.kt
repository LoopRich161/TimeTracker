package ru.looprich.timetracker.presentation.login
...
import ru.looprich.timetracker.theme.colorGreen600
...
fun authentication(
    ...
    if (login.isNotEmpty() and password.isNotEmpty()) {
        val user = app.userRepo.getByLoginAndPassword(login = login, password = password)
        user?.let {
            app.user = user
            Toast.makeText(
                context,
